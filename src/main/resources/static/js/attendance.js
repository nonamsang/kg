...
function initCalendar() {
	...
	if (calendarEl && !calendarInitialized) {
		fetch(`/attendance/dates?user_Id=${encodeURIComponent(userId)}&_=${Date.now()}`, {
			...
			.then(dates => {
				...
				calendar = new FullCalendar.Calendar(calendarEl, {
					...
					// 컴팩트 레이아웃
					height: 'auto',
					contentHeight: 300,   // 더 작게는 260~280으로
				...
				// 과거 출석 데이터 렌더
				norm.forEach((d, idx) => {
					const icons = [isWeekendYMD(d) ? '/img/water2.png' : '/img/water1.png'];
					const count = idx + 1;
					if (count === 10 || count === 20 || count === 30) icons.push('/img/fertilizer.png');
...
function openAttendanceModal() {
	...
	fetch(`/attendance/today?user_Id=${encodeURIComponent(userId)}&_=${Date.now()}`, {
		cache: 'no-store', headers: { 'Accept': 'application/json' }
	})
		.then(r => r.json())
		.then(hasChecked => {
			...
			attendanceBtn.onclick = function() {
				fetch('/attendance/check', {
					...
					.then(data => {
						...
						const icons = [];
						icons.push(data.water === 2 ? '/img/water2.png' : '/img/water1.png');
						if (data.fertilizer) icons.push('/img/fertilizer.png');
						const uniqIcons = Array.from(new Set(icons));
...
// attendance.js — Axios 버전 (CSRF는 레이아웃 인터셉터로 자동 주입됨)
