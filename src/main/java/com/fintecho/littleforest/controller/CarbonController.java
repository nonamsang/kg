...
/**
 * 한 페이지에서: 입력(Form) → 조회(POST) → 출력(View)
 * - 내부 도메인(user_id) => 외부(userId) 변환은 NetZeroPointRequest#toCodefBody에서 수행
 */
...
public class CarbonController {
   ...
   // private final ObjectMapper objectMapper = new ObjectMapper();
    ...
    public String page(Model model) {
        if (!model.containsAttribute("form")) model.addAttribute("form", new NetZeroPointRequest());
        return "carbonpoint";
    }

    @PostMapping("/netzero")
    public String netzero(@ModelAttribute("form") NetZeroPointRequest form, Model model) {
        try {
            ...
            if (form.getYear() != null && !form.getYear().isBlank()) body.put("year", form.getYear());
            ...
            NetZeroPointResponse result =
                om.treeToValue(root.path("data"), NetZeroPointResponse.class);
