...
public class MainController {
	...
	// 회원가입 처리
	   ...
	   public String joinkakao(@RequestParam("email") String email, Model model) {
	       model.addAttribute("email", email);
	   ...
	   @PostMapping("/auth/kakao/session")
	   public ResponseEntity<Void> setupKakaoSession(@RequestBody KakaoTokenRequest request,
	                                                 HttpSession session) {
	       // 1) 카카오 사용자 정보 조회 (토큰 검증 + 이메일 획득)
	       RestTemplate rest = new RestTemplate();
	       HttpHeaders headers = new HttpHeaders();
	       headers.setBearerAuth(request.getAccessToken()); // 사용자 액세스 토큰
	       HttpEntity<Void> entity = new HttpEntity<>(headers);

	       ResponseEntity<Map> kakaoResponse = rest.exchange(
	               "https://kapi.kakao.com/v2/user/me",
	               HttpMethod.GET, entity, Map.class);

	       Map body = kakaoResponse.getBody();
